...
import { Blog_AddForm } from "./BlogAdd";
...
// Correct Blog type based on API
type Blog = {
  blogId?: number;
  title: string;
  slug?: string;
...
export default function BlogAdmin() {
  ...
  const API = `${process.env.NEXT_PUBLIC_API_BASE_URL}/blog`;
  ...
  const loadData = async () => {
    try {
      ...
      // Map API response to Blog type if needed
      const blogs: Blog[] = (json?.data || []).map((b: any) => ({
        blogId: b.blogId,
        title: b.title,
        slug: b.slug,
        excerpt: b.excerpt,
            ...
            <Box sx={{ display: "flex", gap: "0.5rem" }}>
              <Tooltip title="Edit">
                <IconButton
                  color="success"
                  onClick={() =>
                    setModalOpen({ open: true, mode: 2, rowData: row.original as Blog })
                  }
                >
                  <Edit />
                </IconButton>
              ...
              <Tooltip title="View">
                <IconButton
                  color="info"
                  onClick={() =>
                    setModalOpen({ open: true, mode: 4, rowData: row.original as Blog })
                  }
                >
                  <Visibility />
                </IconButton>
              ...
              <Tooltip title="Delete">
                <IconButton
                  color="error"
                  onClick={() =>
                    setModalOpen({ open: true, mode: 3, rowData: row.original as Blog })
                  }
                >
                  <Delete />
                </IconButton>
  ...
  blogData={modalOpen.rowData ?? undefined} // use ?? instead of ||
