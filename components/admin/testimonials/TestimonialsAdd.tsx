...
const Testimonials_AddForm: React.FC<Props> = ({
  mode,
  testimonialData,
  fetchTableData,
  onClose,
  ...
  useEffect(() => {
    if (mode !== 1 && testimonialData) {
      setFormData({
        ...
        createdAt: testimonialData.createdAt || new Date().toISOString(),
      });

      if (testimonialData.photoUrl) {
        const lastPart = testimonialData.photoUrl.split("/").pop() || "";
        ...
        set_doc([
          {
            ...
            fileUri: testimonialData.photoUrl,
          },
        ]);
      }
    }
  ...
  const uploadFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const headers = { Authorization: parsedToken ? `Bearer ${parsedToken}` : "" };
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      fd.append("module", "TESTIMONIALS");
      if (mode !== 1 && testimonialData?.testimonialId) fd.append("entityId", "");

      try {
        const res = await axios.post(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/files/upload?module=TESTIMONIALS`,
          fd,
          { headers }
        );
  ...
  const handleDownload = async (fileUri: string) => {
    ...
    const headers = { Authorization: authTokenValue ? `Bearer ${authTokenValue}` : "" };
  ...
  const delete_file = async (i: number) => {
    ...
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_API_BASE_URL}/files/${fileId}`, { headers });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 1) {
        await axios.post(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/manage/testimonials`,
          payload,
          { headers }
        );
        ...
        await axios.put(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/manage/testimonials/${testimonialData.testimonialId}`,
          payload,
          { headers }
        );
  ...
  const handleDelete = async () => {
    ...
    if (!testimonialData?.testimonialId) return;
    try {
      await axios.delete(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/manage/testimonials/${testimonialData.testimonialId}`,
        { headers }
      ...
      console.error(err);
      setMsg("Failed to delete testimonial!");
      setMsgTyp("error");
    }
  };
    ...
    <div className="container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="row mb-3">
          <label className="col-md-3 form-label">Customer Name:</label>
        ...
        {/* Rating */}
        <div className="row mb-3">
          <label className="col-md-3 form-label">Rating:</label>
          <div className="col-md-8">
            <input
              type="number"
              name="rating"
...
export default Testimonials_AddForm;
