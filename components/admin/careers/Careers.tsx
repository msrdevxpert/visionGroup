...
import { Careers_AddForm } from "./CareersAdd";
...
export default function CareersAdmin() {
  ...
  const API = `${process.env.NEXT_PUBLIC_API_BASE_URL}/careers`;
  ...
  const handleSaveRowEdits = ({ exitEditingMode, row, values }: any) => {
          ...
          <Box sx={{ display: "flex", gap: "0.5rem" }}>
            <Tooltip title="Edit">
              <IconButton
                color="success"
                onClick={() =>
                  setModalOpen({ open: true, mode: 2, rowData: row.original as Career })
                }
              >
                <Edit />
              </IconButton>
            ...
            <Tooltip title="View">
              <IconButton
                color="info"
                onClick={() =>
                  setModalOpen({ open: true, mode: 4, rowData: row.original as Career })
                }
              >
                <Visibility />
              </IconButton>
            ...
            <Tooltip title="Delete">
              <IconButton
                color="error"
                onClick={() =>
                  setModalOpen({ open: true, mode: 3, rowData: row.original as Career })
                }
              >
                <Delete />
              </IconButton>
      ...
      >
        ...
        <DialogContent>
          {modalOpen.open && (
            <Careers_AddForm
              mode={modalOpen.mode}
              careerData={modalOpen.rowData || undefined}
